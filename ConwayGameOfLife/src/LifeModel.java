...
public class LifeModel implements ActionListener
{
    ...
    {
    	updateNextGen();
        // Switch state of cells to their state in the next generation
    ...
    private void updateNextGen() {
    	for(int i = 0; i < myGrid.length; i++) {
            for(int j = 0; j < myGrid[0].length; j++) {
                ...
                // If a cell has 3 living neighbors, it must be alive next gen
                // If it has 2 living neighbors, it keeps the same state
                // Otherwise, the cell dies
                if(numLiveNeighbors == 3) {
                    myGrid[i][j].setAliveNext(true);
                } else if (numLiveNeighbors == 2) {
                    myGrid[i][j].setAliveNext(myGrid[i][j].isAliveNow());
                } else {
